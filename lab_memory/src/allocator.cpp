...
void Allocator::loadRooms(const std::string& file)
{
    ...
    rooms = new Room[roomCount]; //roomCount not initialized
    ...
    while (fileio::areMoreRooms()) {
        ...
        i++;                                 //"i" should be incremented at the end of while loop
    }
}
...
int Allocator::minSpaceRemaining()
{
    int border = 1000000;
    roomCount = fileio::getNumRooms();
    for (int i = 0; i < roomCount; i++)               //roomCount not initialized
